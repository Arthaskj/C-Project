	...
	var util = {
		...
		getParamsUrl:function(){
			var hashDeatail = location.hash.split("?"),
				hashName = "KJ" + (hashDeatail[0].split("#")[1] ? hashDeatail[0].split("#")[1].replace(/\//g,".") : hashDeatail[0].split("#")[1]),//路由地址
	...
	spaRouters.prototype={
		...
		asyncFun:function(file,transition){
		   ...
		   if(self.routers[transition.path].fn){
	           ...
	           scriptEle.onload= function(e,target){
	               self.afterFun && self.afterFun(transition)
	               self.routers[transition.path].fn = kj.RouterOpt[transition.path].beforeSetup;
	               self.routers[transition.path].fn(transition)
	           }
